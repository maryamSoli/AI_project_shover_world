...
class ShoverWorldEnv(gym.Env):
    ...
    def step(self, action: Tuple[int, int, int]):
        ...
        if elapsed_ms >= 200 :  
            penalty_units = int(elapsed_ms // 200)  
            self.stamina = max(0.0, float(self.stamina - penalty_units))  
        ...
        if act_type == 6:
            ...
            if squares:
                ...
                self.stamina += 10 * size * size     
                reward += float(10 * size * size) 
                last_action_valid = True
                self._sync_perf_squares_initial()
                self._rebuild_stationary()
            else:
